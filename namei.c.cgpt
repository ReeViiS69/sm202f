                       ...
                       bool got_write, int *opened)
{
    ...
    *opened &= ~FILE_CREATED;
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
    if (unlikely(dentry->d_inode && dentry->d_inode->i_state & INODE_STATE_SUS_PATH) &&
    ...
    if ((nd->flags & LOOKUP_OPEN) && dir_inode->i_op->atomic_open) {
        error = atomic_open(nd, dentry, path, file, op, got_write,
                            need_lookup, opened);
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
        if (!IS_ERR(dentry) && dentry->d_inode &&
            ...
            likely(current->susfs_task_state & TASK_STRUCT_NON_ROOT_USER_APP_PROC)) {
            if (error == -ENOENT)
                return error; // already returning error
            dput(dentry);
            return -ENOENT;
        }
#endif
